...
export default function TemplateGalleryPage() {
  ...
  const handleBusinessSetupSubmit = (profile: BusinessProfile) => {
    if (!businessSetupModal.templateConfig) return;
    const patched = injectBusinessProfileIntoConfig(
      businessSetupModal.templateConfig,
      profile
    );
    const brand = profile.brandName?.trim() || undefined;
    setBusinessSetupModal({ open: false, templateConfig: null });
    handleCreateFromTemplate(patched, brand);
  };

  const handleBusinessSetupSkip = () => {
    if (!businessSetupModal.templateConfig) return;
    const cfg = businessSetupModal.templateConfig;
    setBusinessSetupModal({ open: false, templateConfig: null });
    handleCreateFromTemplate(cfg);
  };
  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC] text-slate-900 flex flex-col font-sans">
      ...
      {previewTemplate && (
        <div
          className="fixed inset-0 z-50 bg-slate-950/80 backdrop-blur-md flex items-center justify-center p-2 sm:p-6 animate-fade-in"
        ...
        >
          ...
          >
            {/* Modal Header */}
            <div className="flex items-center justify-between px-4 sm:px-6 py-3.5 border-b border-slate-800 bg-slate-900/90 backdrop-blur z-10 shrink-0">
              ...
              <div className="flex items-center gap-2">
                ...
                <button
                  onClick={() => {
                    const cfg = previewTemplate.config;
                    const name = previewTemplate.name;
      ...
      <QuickBusinessSetupModal
        isOpen={businessSetupModal.open}
        onClose={() => setBusinessSetupModal({ open: false, templateConfig: null })}
        templateName={businessSetupModal.templateName || "Template"}
